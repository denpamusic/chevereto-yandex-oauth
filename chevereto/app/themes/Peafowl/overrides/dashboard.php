	...
	<div class="form-content">
		...
		<form id="dashboard-settings" method="post" data-type="<?php echo get_dashboard(); ?>" data-action="validate" enctype="multipart/form-data">
			...
			<div class="input-label">
				...
				<div class="c5 phablet-c1"><select type="text" name="yandex" id="yandex" class="text-input" data-combo="yandex-combo">
					<?php
						echo CHV\Render\get_select_options_html([1 => _s('Enabled'), 0 => _s('Disabled')], get_safe_post() ? get_safe_post()['yandex'] : CHV\Settings::get('yandex'));
					?>
				</select></div>
				<div class="input-below"><?php _se('You need a <a href="https://oauth.yandex.ru" target="_blank">Yandex app</a> for this.'); ?></div>
			</div>
			<div id="yandex-combo">
				<div data-combo-value="1" class="switch-combo c9 phablet-c1<?php if(!(get_safe_post() ? get_safe_post()['yandex'] : CHV\Settings::get('yandex'))) echo ' soft-hidden'; ?>">
					<div class="input-label">
						<label for="yandex_client_id"><?php _se('Yandex client id'); ?></label>
						<input type="text" name="yandex_client_id" id="yandex_client_id" class="text-input" value="<?php echo get_safe_post() ? get_safe_post()['yandex_client_id'] : CHV\Settings::get('yandex_client_id', true); ?>">
						<div class="input-warning red-warning"><?php echo get_input_errors()['vk_client_id']; ?></div>
					</div>
					<div class="input-label">
						<label for="yandex_client_secret"><?php _se('Yandex client secret'); ?></label>
						<input type="text" name="yandex_client_secret" id="yandex_client_secret" class="text-input" value="<?php echo get_safe_post() ? get_safe_post()['yandex_client_secret'] : CHV\Settings::get('yandex_client_secret', true); ?>">
						<div class="input-warning red-warning"><?php echo get_input_errors()['yandex_client_secret']; ?></div>
					</div>
				</div>
			</div>
			<?php } ?>
